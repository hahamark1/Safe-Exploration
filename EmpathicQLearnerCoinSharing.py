...
class EmphaticQLearner():
    ...
    def __init__(self, load_q_values=False):
        self.Q_values = {}
        ...
        self.env = GridworldCoinSharingGym(step_reward=self.step_reward,
    ...
    def do_game_step(self, move):
        ...
        if done:
            ...
            self.log_scalar('coins_collected', self.coins_collected[-1], self.step)
            self.log_scalar('enemy_coins_collected', self.enemy_coins_collected[-1], self.step)
            self.log_scalar('equality', min(self.coins_collected[-1], self.enemy_coins_collected[-1])/max(self.coins_collected[-1], self.enemy_coins_collected[-1]), self.step)
            ...
            self.log_scalar('mean_enemy_q', np.mean(self.log_enemy_q_values[-1]), self.step)
            self.log_histogram('q_values', np.array(self.log_q_values[-1]), self.step, 20)
            self.log_histogram('enemy_q_values', np.array(self.log_enemy_q_values[-1]), self.step, 20)
            self.rewards.append(0)
            self.enemies_killed.append(0)
            self.got_killed.append(0)
            self.enemy_coins_collected.append(0)
            self.coins_collected.append(0)
            self.log_q_values.append([])
            self.log_enemy_q_values.append([])
    ...
    def get_best_action(self):
        ...
        for action in range(4):
            ...
            if action not in self.Q_values[enemy_level_key]:
                self.Q_values[enemy_level_key][action] = 0
    ...
    def Q_learning(self):
        ...
        self.coins_collected[-1] += info['coins_collected']
